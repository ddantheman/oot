...
def remove_stale_assets(root: str, copied_files: T.List[str], last_cache_time: int):
    ...
    for file in stale:
        ...
        pattern = os.path.join(root, sys.argv[2], os.path.dirname(rel), "*")
...
def remove_stale_actors(root: str, last_cache_time: int):
    ...
    actors_build_dir = os.path.join(root, sys.argv[2], "src", "overlays", "actors")
...
def update_cache_time(root: str) -> int:
    build_dir = os.path.join(root, sys.argv[2])
